...
@Component({
  selector: 'app-view-result-number',
  templateUrl: './view-result-number.component.html',
  styleUrl: './view-result-number.component.scss'
})
export class ViewResultNumberComponent implements OnInit{
  lotteries:string[] = [];
...
buildForm(){
  ...
  this.formSearchResult = this.formBuilder.group({
    lotterie: [null],
    contest: [null],
  });
}
...
getResult(){
  this.stateService.setIsLoading(true);
  var lotterie = this.formSearchResult.get('lotterie').value;
  var contest = this.formSearchResult.get('contest').value;
  this.lotteriesResulService.getEspecifiedResultForTypeLotterie(lotterie, contest).subscribe({
    next: (data) => {
    ...
    error: (e) => {
      this.showToastrService.showError('Houve um erro ao buscar o resultado, confira se essa sorteio já aconteceu');
      this.stateService.setIsLoading(false);
    }
  })
}
